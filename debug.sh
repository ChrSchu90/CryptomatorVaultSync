...
docker buildx build --load --progress=plain --platform ${DOCKER_PLATFORM} --build-arg CRYPTOMATOR_CLI_VERSION=${CRYPTOMATOR_CLI_VERSION} --build-arg RCLONE_RELEASE=${RCLONE_RELEASE} -f ${DOCKER_FILE} -t ${IMAGE_NAME} . && \
